...
def save_results(df, file_name = None, timestamped = False):
    ...
    if file_name is not None and file_name[-4:]==".csv":
        # remove extension
        file_name = file_name[:-4]

    if file_name is None:
        file_name = directory+"Results_{}.csv".format(time.strftime("%Y-%m-%d-%H%M"))
    elif timestamped:
        file_name = directory + file_name + "_"+time.strftime("%Y-%m-%d-%H%M") + ".csv"
    else:
